...
import { getCategoryPosts } from '@/lib/actions/trending';
import { CategoryDef } from '@/lib/constants/categories';
...
export default function CategoryDetailClient({
  ...
  const handlePeriodChange = async (newPeriod: 'week' | 'month' | 'all') => {
    ...
    setLoading(true);
    try {
      const res = await getCategoryPosts(category.slug, newPeriod);
      if (res) {
        setPosts(res.posts);
      }
    } catch (e) {
  ...
  return (
    <div className="categories-page">
      <div className="categories-layout" style={{ paddingTop: '32px' }}>
        <main className="categories-main">
          {/* Banner */}
          <div className="category-detail-banner">
            <div className="category-detail-icon">{getCategoryIcon(category.icon)}</div>
          ...
          <div className="trending-posts-list">
            {loading ? (
              ...
              <div className="empty-state">ยังไม่มีโพสต์ในหมวดหมู่นี้ในขณะนี้</div>
            ) : (
              posts.map((post) => (
